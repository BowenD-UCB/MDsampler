...
def get_train_val_test_loader(
    ...
    **kwargs,
    ...
    test_size = (
        kwargs["test_size"] if kwargs["test_size"] else int(test_ratio * total_size)
    )
    valid_size = (
        kwargs["val_size"] if kwargs["val_size"] else int(val_ratio * total_size)
    )
...
def collate_pool(dataset_list):
    ...
    batch_cif_ids = []
    base_idx = 0
    for _i, ((atom_fea, nbr_fea, nbr_fea_idx), target, cif_id) in enumerate(
        dataset_list
    ):
...
class AtomCustomJSONInitializer(AtomInitializer):
    """Initialize atom feature tensors using a JSON file, which is a python
...
class CIFData(Dataset):
    ...
    def __init__(
        ...
        self.dmin, self.step = float(dmin), float(step)
